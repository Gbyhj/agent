...
class MCPServer:
    ...
    def run_stdio(self):
        ...
        import sys
        log.debug(f"MCP Server '{self.name}' v{self.version} running on stdio", file=sys.stderr)
        for line in sys.stdin:
            ...
            try:
                ...
                log.debug(json.dumps(response), flush=True)
            except json.JSONDecodeError:
                log.debug(json.dumps({"jsonrpc": "2.0", "error": {"code": -32700, "message": "Parse error"}}), flush=True)
